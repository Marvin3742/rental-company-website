...
import { claimDiscountCode } from "./discounts.js";
 ...
 */
export async function createBookingHold({ eventDate, lines, customer, paymentMode, discountCode }) {
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      // Claim the discount code (if any) only once we know the booking will
      // actually be created — a shortfall/blackout failure above rolls back
      // the whole transaction, so the code is never consumed for a booking
      // that didn't happen.
      let discountCodeId = null;
      let discountCents = 0;
      if (discountCode) {
        const claim = await claimDiscountCode(discountCode, subtotalCents + deliveryFeeCents, tx);
        discountCodeId = claim.discountCodeId;
      ...
      return { booking, totalCents, depositCents, chargeNowCents, balanceDueCents, discountCents };
    },
  ...
  return {
    ...
    chargeNowCents: result.chargeNowCents,
...
export async function createManualBooking({
  ...
  if (!["UPCOMING", "COMPLETED"].includes(status)) {
    throw new HttpError(400, "status must be UPCOMING or COMPLETED");
  }
