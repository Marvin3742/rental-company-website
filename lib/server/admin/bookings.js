...
export default withApi({
  async GET(req) {
    ...
    // "Upcoming" view sorts soonest-first (event date ascending); every other
    // view sorts by most recent event date first (descending). createdAt breaks
    // ties. Reference search ignores the status filter, so it stays descending.
    const upcomingView = status === "UPCOMING" && !(ref && ref.trim());
    const bookings = await prisma.booking.findMany({
      where,
      orderBy: [{ eventDate: upcomingView ? "asc" : "desc" }, { createdAt: "desc" }],
