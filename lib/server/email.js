...
function shell(title, bodyHtml) {
  ...
  <div style="max-width:560px;margin:0 auto;padding:24px;">
    <div style="background:#1d4ed8;color:#fff;padding:16px 20px;font-size:18px;font-weight:700;">${esc(business.name)}</div>
...
export function buildCustomerEmail(b) {
  const body = `
    ...
    <h3 style="font-size:14px;margin:18px 0 4px;">Your items</h3>
    ${table(lineRows(b.lines))}
    <h3 style="font-size:14px;margin:18px 0 4px;">Payment</h3>
    ${table(totalsRows(b))}
    <h3 style="font-size:14px;margin:18px 0 4px;">Delivery to</h3>
...
export function buildOwnerEmail(b) {
  const body = `
    ...
    <h3 style="font-size:14px;margin:18px 0 4px;">Customer</h3>
    <p style="margin:0;">${esc(b.customerName)}<br>${esc(b.customerPhone)} · ${esc(b.customerEmail)}</p>
    <h3 style="font-size:14px;margin:18px 0 4px;">Deliver to</h3>
    <p style="white-space:pre-line;margin:0;">${esc(b.deliveryAddress)}</p>
    ...
    <h3 style="font-size:14px;margin:18px 0 4px;">Items</h3>
    ${table(lineRows(b.lines))}
    <h3 style="font-size:14px;margin:18px 0 4px;">Payment</h3>
    ${table(totalsRows(b))}
    <p style="margin-top:20px;"><a href="${SITE_URL}/admin" style="background:#1d4ed8;color:#fff;padding:10px 16px;text-decoration:none;">Open admin dashboard</a></p>`;
