...
export function signAdminToken(payload) {
  return jwt.sign(payload, getSecret(), { algorithm: "HS256", expiresIn: MAX_AGE_SECONDS });
}
...
export function requireAdmin(req) {
  ...
  try {
    // Pin the algorithm so a token can't downgrade verification to a weaker
    // or mismatched scheme than the one we sign with.
